...
Legend-State currently ships with local plugins for Local Storage and IndexedDB on web and `react-native-mmkv` in React Native, and a remote plugin for Firebase is coming soon.
...
We are working on a remote persistence plugin for Firebase for both web and React Native. It is more complex, because it include conflicts resolution and extensive optimization, so it needs some more work before it's ready for release. Please [let us know](https://github.com/LegendApp/legend-state/issues/5) if you'd like us to hurry up with that.
...
Remote persistence plugins are more complex, and we have not quite locked down the API as we're finishing the Firebase plugin and building it into our apps. If you'd like to build your own please [post a GitHub issue](https://github.com/LegendApp/legend-state/issues) and we'll work with you on it.

...
```jsx
import { useIsMounted } from "@legendapp/state/react-hooks/useIsMounted"
...
`useObservableQuery` is an observable wrapper around [TanStack Query](https://tanstack.com/query/v4/?from=reactQueryV3&original=https://react-query-v3.tanstack.com/), which is used in the exact same way as `useQuery` but it returns an observable and does not re-render on changes.
...
function Component() {
    ...
    return (
        <div>
            <h2>{data.email}</h2>
            <div>{isFetching ? 'Updating...' : ''}</div>
            <button onClick={() => obs.data.email.set('testemail')}>Change email</button>
...
See [CodeSandbox](https://codesandbox.io/s/legend-state-useobservablequery-0w1yil?file=/src/basic.tsx) for an interactive example.

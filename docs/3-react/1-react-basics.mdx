...
const Component = ({ id }) => {
    // Track all observables accessed with a selector function
    const isSelected = useSelector(() => id === state.selected.get())
    // Only re-renders if the return value changes

    return <div>Is selected: {isSelected}</div>
}

const App = ({ id }) => {
    // Track a single observable directly
    const selected = useSelector(state.selected)
    // Only re-renders if the return value changes
...
<Sandbox height={480} width={64} options={{ showTabs: true }} files={{
'/App.js': `
import { useRef } from 'react'

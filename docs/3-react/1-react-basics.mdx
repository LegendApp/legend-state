...
- Rendering an observable directly creates a memoized auto-tracking component for its contents. This can be an optimization so the parent doesn't need to re-render when it changes, but if the parent component is already tracking that observable anyway, it's wasteful so you should render the value rather than the observable.
- `useSelector` is the most efficient option so if a component only needs one value from observables it's the optimal way to go. If it's consuming multiple observables then reusing the `forceRender` helps, and the benefit over `observer` becomes less significant.
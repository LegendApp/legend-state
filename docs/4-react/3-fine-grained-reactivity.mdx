...
const Normal = observer(function Normal() {
...
const ComputedExample = observer(() => {
    const renderCount = ++useRef(0).current
...
Memo is similar to Computed, but it will never re-render from parent changes - only if its own observables change. Use `Memo` when children are truly indepdendent from the parent component. This is equivalent to extracting it as a separate `observer` function.
...
Props:
- `if`: A computed function or an observable (use `obs()` to get the observable from a primitive)
...
Props:
- `value`: A computed function or an observable (use `obs()` to get the observable from a primitive)
- `children`: An object with the possible cases of `value` as keys. If `value` doesn't match any of the cases it will use the `default` case if it exists.
...
```jsx
<Computed><div>Count: {state.count}</div></Computed>

...
Version 2.0 is still beta, but version 1.11 displays deprecation warnings to help you migrate.
...
When setting a Promise into an observable it retains the Promise until it resolves or rejects so that you can check the status on it. After it resolves or rejects it replaces itself with the resolved value or the error. Previously it set undefined until it resolved or rejected.

So if you had logic to check whether a Promise was resolved that depending on `undefined` meaning not yet resolved, you can change that to check whether it's a Promise.
...
To support taking an initial state or an observable, `persistObservable` needs to return both an observable and the syncState, so it now returns an array.

See [persistObservable](../persistence#persistobservable) for details.

```js
const [obs$, syncState] = persistObservable(initialStateOrObservable, { ... })
```
...
Since there were not any remote persistence plugins before, these changes would likely not affect you unless you made your own. See [Remote Persistence](../persistence#remote-persistence) for details.

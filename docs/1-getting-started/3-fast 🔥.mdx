...
Extensive care is taken to ensure that components are rendered only when their state truly changes. Legend-State provides [functions](../tracking) to be extra specific about when it tracks changes and [useComputed](../useComputed) to isolate a tracking computation to return one value. The best thing for your app's performance is to ensure that state changes cause minimal component renders.

...
describe('Set', () => {
    test('Set proxy', () => {
        const obs = observable({ test: { text: 't' }, data: [] });
        const handler = jest.fn();
        // @ts-ignore
        obs.test.onChange(handler);
        const val = obs.test.text;
        ...
        // @ts-ignore
        obs.data.set(['hi', 'hello']);
        expect(obs.data.map((a) => a)).toEqual(['hi', 'hello']);
        expect(obs.data.length).toEqual(2);

        debugger;
        // @ts-ignore
        obs.test.text.set('t3');
    });
    //     test('Set', () => {
    ...
    //         const computed = observableComputed([obs._.prop('test'), obs._.prop('test2')], (test, test2) => test + test2);

...
interface TwoWayComputedObservable<T, T2> extends ImmutableObservableBase<T>, MutableObservableBase<T2> {}
...
type NonObservableKeys<T> = {
    [K in keyof T]-?: T[K] extends undefined | null ? never : NonNullable<T[K]> extends NonObservable ? K : never;
}[keyof T];
...
type ObservableNode<T, NT = NonNullable<T>> = [NT] extends [never] // means that T is ONLY undefined or null
    ...
    : ObservableObject<ObservableProps<T>> & ObservableChildren<ObservableProps<T>> & NonObservableProps<T>;

type Simplify<T> = { [K in keyof T]: T[K] } & {};
type Observable<T> = ObservableNode<T>;

...
import { MappedProxyValue, ObsProxyUnsafe } from './ObsProxyInterfaces';

function onChanged(proxy: ObsProxyUnsafe, args: ObsProxyUnsafe[], compute: (...args: any) => any) {
    const value = compute(...args.map((a) => a.value));
    proxy.value = value;
}

export function obsProxyComputed<T, TA extends ObsProxyUnsafe[]>(
    args: TA,

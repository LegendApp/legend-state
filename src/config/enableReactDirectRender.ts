...
export function enableReactDirectRender() {
    if (process.env.NODE_ENV === 'development') {
        console.warn(
            '[legend-state] enableReactDirectRender is deprecated and will be removed in version 3.0. Please convert it from {value} to <Memo>{value}</Memo>. See https://legendapp.com/open-source/state/migrating for more details.',
        );
    }
    if (!isEnabled) {
        isEnabled = true;
        ...
        const Text = memo(function Text({ data }: { data: ObservableReadable }) {
            if (process.env.NODE_ENV === 'development') {
                if (isObservable(data)) {
                    console.warn(
                        `[legend-state] enableReactDirectRender is deprecated and will be removed in version 3.0. Please convert rendering of observable with path ${getNodePath(
                            getNode(data),
